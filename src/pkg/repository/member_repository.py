...
class MemberRepository(BaseRepository):
    ...
    def search(self, badge="", tier="Tutte", name="", surname="", phone="", limit=50, offset=0, sort_by="scheda", sort_order="asc"):
        with self._get_session() as db:
            ...
            # Mappatura delle colonne per l'ordinamento
            sort_columns = {
                "scheda": Member.badge_number,
                "nome": Member.first_name,
                "cognome": Member.last_name,
                "fascia": Tier.name,
                "scadenza": Member.membership_expiration,
                "scad_iscr": Member.enrollment_expiration
            }
            
            sort_column = sort_columns.get(sort_by, Member.badge_number)
            
            # Se si ordina per fascia, serve la join con Tier
            if sort_by == "fascia" and tier == "Tutte":
                query = query.outerjoin(Tier)
            elif tier != "Tutte" and sort_by != "fascia":
                query = query.join(Tier)
            
            if sort_order == "desc":
                query = query.order_by(sort_column.desc(), Member.id)
            else:
                query = query.order_by(sort_column.asc(), Member.id)
            
            members_db = query.offset(offset).limit(limit).all()
