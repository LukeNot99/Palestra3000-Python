...
from src.pkg.utility.utils import parse_date, is_valid_date, is_valid_phone, calculate_partial_cf, generate_invoice_html

class MemberFormWindow(ctk.CTkToplevel):
    def __init__(self, parent, app, refresh_callback, member_id=None):
        super().__init__(parent)
        self.title("Scheda Socio")
        self.geometry("950x700") 
        self.configure(fg_color=("#F2F2F7", "#1C1C1E"))
        self.app = app
        self.member_repository = self.app.di.get_member_repository()
        self.tier_repository = self.app.di.get_tier_repository()
        ...
        unique_cities, unique_birth_places = self.member_repository.get_unique_cities_and_birthplaces()
        self.tiers_data = self.tier_repository.get_all()
        ...
        ctk.CTkLabel(frame_right, text="Dati Medici:", font=ctk.CTkFont(family="Montserrat", weight="bold"), text_color=("#86868B", "#98989D")).pack(anchor="w", padx=20, pady=(0,10))
    ...
    def generate_receipt(self):
        ...
        socio_data = self.member_repository.get_by_id(self.member_id)
        if socio_data:
    ...
    def load_member_data(self):
        socio = self.member_repository.get_by_id(self.member_id)
        if socio:
    ...
    def save_member(self, exit_after=True):
        ...
        self.member_id = self.member_repository.save(data, self.member_id)
...
class MembersView(ctk.CTkFrame):
    def __init__(self, parent, app, **kwargs):
        ...
        self.sort_by = "scheda"  # Ordinamento di default per tessera
        self.sort_order = "asc"  # Ordine crescente di default
        ...
        tiers = self.tier_repository.get_all()
        ...
        for i, col in enumerate(self.cols):
            ...
            # Rendi l'header cliccabile per ordinare
            header_label = ctk.CTkLabel(header_frame, text=col[1], font=ctk.CTkFont(family="Montserrat", size=12, weight="bold"), text_color=("#86868B", "#98989D"), anchor=col[3], cursor="hand2")
            header_label.grid(row=0, column=i, padx=10, pady=5, sticky="ew")
            # Aggiungi evento click per ordinamento
            header_label.bind("<Button-1>", lambda e, c=col[0]: self.toggle_sort(c))
    ...
    def toggle_sort(self, column):
        """Cambia l'ordinamento quando si clicca sull'header della colonna"""
        if self.sort_by == column:
            # Inverti l'ordine se clicco sulla stessa colonna
            self.sort_order = "desc" if self.sort_order == "asc" else "asc"
        else:
            # Nuova colonna: ordina crescente di default
            self.sort_by = column
            self.sort_order = "asc"
        self.load_data(reset_page=True)
    ...
    def load_data(self, reset_page=False):
        ...
        members_data, total_members = self.member_repository.search(
    ...
    def open_new_form(self):
        MemberFormWindow(self, self.app, refresh_callback=lambda: self.load_data(reset_page=False))
    ...
    def delete_member(self):
        ...
        if messagebox.askyesno("Conferma", "Vuoi eliminare definitivamente questo socio?"):
            self.member_repository.delete(self.selected_member_id)
            self.load_data(reset_page=False)
