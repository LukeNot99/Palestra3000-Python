...
class TiersView(ctk.CTkFrame):
    def __init__(self, parent, controller=None, **kwargs):
        ...
        self.tier_repository = self.controller.di.get_tier_repository()
    ...
    def load_data(self):
        ...
        tariffe = self.tier_repository.get_all()
        for t in tariffe:
    ...
    def load_into_form(self):
        ...
        tariffa = self.tier_repository.get_by_id(self.selected_tier_id)
        if tariffa:
            self.clear_form()
    ...
    def save_tier(self):
        ...
        self.tier_repository.save(data, self.tier_id_in_modifica)
        self.clear_form()
        self.load_data()
    ...
    def delete_tier(self):
        ...
        soci_collegati = self.tier_repository.count_linked_members(self.selected_tier_id)
        if soci_collegati > 0: 
        ...
        if messagebox.askyesno("Conferma", "Sei sicuro di voler eliminare questa fascia?"):
            self.tier_repository.delete(self.selected_tier_id)
            self.clear_form()
            self.load_data()
